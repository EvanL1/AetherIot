...
image=${AETHER_PV_FORECASTING_IMAGE:-}
token=${AETHER_PV_FORECASTING_BEARER_TOKEN:-}
concurrency=${AETHER_PV_FORECASTING_MAX_CONCURRENCY:-1}
bundles=${AETHER_PV_FORECASTING_ARTIFACT_BUNDLES:-}

[[ $image =~ ^[^[:space:]]+@sha256:[0-9a-f]{64}$ ]] || \
    fail "AETHER_PV_FORECASTING_IMAGE must be pinned as name@sha256:<64 lowercase hex>"
[[ ${#token} -ge 32 && ${#token} -le 8192 ]] || \
    fail "AETHER_PV_FORECASTING_BEARER_TOKEN must contain 32..8192 characters"
[[ $token =~ ^[A-Za-z0-9._~+/=-]+$ ]] || \
    fail "AETHER_PV_FORECASTING_BEARER_TOKEN contains unsupported characters"
[[ $concurrency =~ ^[0-9]+$ ]] || \
    fail "AETHER_PV_FORECASTING_MAX_CONCURRENCY must be an integer"
(( concurrency >= 1 && concurrency <= 256 )) || \
    fail "AETHER_PV_FORECASTING_MAX_CONCURRENCY must be inside [1, 256]"
[[ -n $bundles ]] || fail "AETHER_PV_FORECASTING_ARTIFACT_BUNDLES is required"
...
try:
    ...
    fail("AETHER_PV_FORECASTING_ARTIFACT_BUNDLES must be valid JSON")
if not isinstance(declarations, list) or not 1 <= len(declarations) <= 128:
    fail("AETHER_PV_FORECASTING_ARTIFACT_BUNDLES must be a non-empty array of at most 128 bundles")
...
echo "PV-Forecasting production environment is structurally valid"
