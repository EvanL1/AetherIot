...
These examples isolate the request-driven PV-Forecasting adapter from the
default Aether runtime. They are commissioning assets, not default services.

## Compose profile

Build or load an image containing the existing PV-Forecasting Edge-Platform,
...
```bash
export AETHER_PV_FORECASTING_IMAGE=registry.example/pv-forecasting@sha256:<digest>
export AETHER_PV_FORECASTING_BEARER_TOKEN='<at least 32 allowed characters>'
export AETHER_PV_FORECASTING_ARTIFACT_BUNDLES='<strict commissioned JSON array>'
integrations/pv-forecasting/deploy/validate-production-env.sh
docker compose \
  -f docker-compose.yml \
  -f integrations/pv-forecasting/deploy/docker-compose.data-processing.yaml \
  --profile data-processing \
  up -d aether-pv-forecasting-processor aether-api
```
...
Commission the sidecar with `AETHER_PV_FORECASTING_REQUIRE_AUTH=true`, a
secret `AETHER_PV_FORECASTING_BEARER_TOKEN`, a bounded
`AETHER_PV_FORECASTING_MAX_CONCURRENCY`, and the strict
`AETHER_PV_FORECASTING_ARTIFACT_BUNDLES` JSON described in the parent
...
available as `/opt/pv-forecasting/app.py` and its locked `uv` environment is
at `/opt/pv-forecasting/.venv`:
...
`/etc/aether/pv-forecasting-processor.env` owned by root with mode `0600` and
set `AETHER_PV_FORECASTING_REQUIRE_AUTH=true` so a missing secret prevents
